...
while True:
    ...
    for cnt in contours:
        ...
        if len(approx) > 5 and perimeterMin < perimeter < perimeterMax:
            ...
            if cv2.contourArea(cnt) < 300:
                # Get contour's centroid and push its RGB color into cnt_colors list
                M = cv2.moments(cnt)
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                cnt_colors.append(hsv[cY][cX])
                s_min = cnt_colors[0][1]
                v_max = 0
                # Check if the color of the centroid same as the ball color
                for color in cnt_colors:
                    _, s, v = color[0], color[1], color[2]
                    if s < s_min and v > v_max:
                        s_min = s
                        v_max = v
    ...
    # hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    # contoured_mask = cv2.bitwise_and(hsv, hsv, mask=finalThresholdImage)
    # mean = cv2.mean(frame, mask=contoured_mask)

    # h, s, v = cv2.split(contoured_mask)
    # flatten = np.bincount(h.flatten())

    # print(real_cnts)
    cv2.imshow("cnt", contoured)
